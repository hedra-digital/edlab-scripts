...
def process_tree(directory, max_digits, dry_run, show_changes, prefix=None, suffix=None):
    output = []
...
def main():
    ...
    # Verifica o diretório do primeiro arquivo ou diretório para encontrar o maior número de dígitos
    ...
    # Processa e mostra a árvore de mudanças
    for target_path in target_paths:
        if target_path.is_dir():
            tree_output = process_tree(target_path, max_digits, args.dry_run, args.show_changes, prefix=args.prefix_pattern, suffix=args.suffix_pattern)
            print(tree_output)
...
def process_file(filepath, max_digits, dry_run, show_changes):
    original_filename = filepath.name
    new_filename = align_place_values(original_filename, max_digits)
...
# Função para exibir a árvore de diretórios e as mudanças planejadas
def show_tree(directory, max_digits, prefix=None, suffix=None):
    output = []
    def print_tree(root, prefix_str=""):
        contents = list(Path(root).iterdir())
        pointers = ['├── '] * (len(contents) - 1) + ['└── ']
...
def main():
    ...
    # Mostra a árvore de mudanças
    if args.show_changes:
        for target_path in target_paths:
            if target_path.is_dir():
                tree_output = show_tree(target_path, max_digits, prefix=args.prefix_pattern, suffix=args.suffix_pattern)
                print(tree_output)
        return  # Saímos aqui para não fazer alterações reais quando estamos apenas mostrando as mudanças

    # Processa cada arquivo ou diretório especificado
    for target_path in target_paths:
        if target_path.is_file():
            if (args.prefix_pattern and not target_path.name.startswith(args.prefix_pattern)) or \
               (args.suffix_pattern and not target_path.name.endswith(args.suffix_pattern)):
                continue
            process_file(target_path, max_digits, args.dry_run, args.show_changes)
                    ...
                    for file in files:
                        ...
                        if (args.prefix_pattern and not file.startswith(args.prefix_pattern)) or \
                           (args.suffix_pattern and not file.endswith(args.suffix_pattern)):
                            continue
                        process_file(filepath, max_digits, args.dry_run, args.show_changes)
            else:
