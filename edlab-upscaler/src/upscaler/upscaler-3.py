...
def split_tensor(tensor, max_splits=3):  # Reduzir para 3 partes
...
def process_image(input_path, model_path, device, scale, target_dpi=300, target_width_mm=None, num_processes=None):
    try:
        # Reduzir o número de workers para 2
        ...
        max_dimension = 2000  # Reduzir o limite de redimensionamento
        ...
        with ThreadPoolExecutor(max_workers=num_processes) as executor:
            process_func = partial(process_tensor_part, 
                                   ...
                                   device=device,
                                   scale=scale)
            outputs = list(executor.map(process_func, parts_data))
